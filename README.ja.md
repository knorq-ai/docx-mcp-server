...
|------|-----------|------------|
| 操作あたりの出力トークン | **80–95% 削減** | ベースライン (エージェントがコード全体を生成) |
| 操作あたりのコスト | **70–90% 削減** | ベースライン |
| 損益分岐点 | **2 操作** | — |
...
|--------|-----|---------------------|--------|
| 段落 0–20 の読み取り | ~30 | ~250 (open, iterate, print) | **88%** |
| テキスト検索・置換 | ~60 | ~350 (regex, run 走査) | **83%** |
| 変更履歴付き挿入 | ~50 | ~600 (w:ins XML 構築, datetime, 著者, run プロパティ) | **92%** |
| テキストへのコメント追加 | ~50 | ~700 (comment part, アンカーマーカー, relationship, XML 操作) | **93%** |
| テキスト書式 (太字 + 色) | ~50 | ~400 (run 検索, 境界分割, rPr 適用) | **88%** |
| 段落書式 (3 段落) | ~60 | ~350 (読み込み, インデックス解決, プロパティ設定, 保存) | **83%** |
| 複合: 読み取り → 検索 → 編集 → コメント | ~190 | ~1,800 | **89%** |

特に **変更履歴** と **コメント** で削減効果が大きい。python-docx にはこれらの組み込み API がないため、エージェントは生の OOXML 操作コードを生成する必要がある (~500–700 出力トークン/操作)。MCP ツールはこの複雑さを内部で処理し、シンプルなパラメータ呼び出しで済む。

出力トークンは入力トークンの 5 倍の単価であるため、コード生成の省略はコストに大きく影響する。32 ツールのスキーマオーバーヘッド (~2,500 トークン) は 2 操作で回収できる。
